...
const AppointmentForm = ({ selectedDate, initialData, onSubmit, onClose }) => {
  ...
  // Helper function to combine the selected date with the chosen time slot
  const combineDateAndTime = (date, timeStr) => {
    const [hours, minutes] = timeStr.split(':');
    const combinedDate = new Date(date); // Copy the selectedDate (which has time set to midnight)
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...formData,
      service: serviceName,
      // Combine the selected date with the chosen time slot before sending
      date: combineDateAndTime(selectedDate, formData.timeSlot).toISOString()
    });
  };
